...
class EmployeeList extends Component {
  ...
  addEmployee = () => {
    ...
    payroll.addEmployee(address, salary, {from:account,gas:3000000}).then(() => {
      const newEmployee = {
        address,
        salary,
        key:address,
        lasypaidDay: new Date().toString()
      };
  ...
  updateEmployee = (address, salary) => {
    ...
    payroll.updateEmployee(address, salary, {from:account, gas:3000000}).then(() => {
      this.setState({
  ...
  removeEmployee = (employeeId) => {
    const {payroll, account} = this.props;
    const {employees} = this.state;
    payroll.removeEmployee(employeeId, {from:account, gas:3000000}).then(() => {
      this.setState({
