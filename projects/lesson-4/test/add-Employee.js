...
contract('Payroll', function (accounts) {
  ...
  it("Test addEmployee() employee already exist ", function () {
    var payroll;
    return Payroll.new().then(function (instance) {
        payroll = instance;
        return payroll.addEmployee(employee, salary, {from: owner});
    }).then(() => {
        assert.equal(addEmployee.employee, employee, "Should not be successful");
    }).catch(error => {
        assert.include(error.toString(), "Error: VM Exception", "the employee already exist");
    });     
  });
});
